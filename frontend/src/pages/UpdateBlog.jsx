...
const UpdateBlog = () => {
  ...
  const subtitleAutoGeneratedRef = useRef(false);
  ...
  const getAutoCategory = (title = "") => {
    const value = title.toLowerCase();

    const categoryKeywords = {
      tech: [
        "javascript", "react", "reactjs", "node", "nodejs", "express",
        "mongodb", "api", "coding", "programming", "software", "developer",
        "development", "frontend", "backend", "full stack", "fullstack",
        "html", "css", "typescript", "python", "java", "git", "github",
        "database", "cloud", "cyber", "technology", "web", "app", "nextjs",
        "next.js", "vue", "angular", "sql", "docker", "kubernetes"
      ],
      "generative-ai": [
        "generative ai", "genai", "image generation", "text generation",
        "ai generated", "ai generation"
      ],
      llms: [
        "llm", "llms", "large language model", "chatgpt", "gemini",
        "claude", "transformer", "prompt engineering", "rag"
      ],
      business: [
        "business", "startup", "marketing", "finance", "investment",
        "money", "entrepreneur", "sales", "company", "management",
        "career", "leadership", "strategy", "profit", "freelance"
      ],
      lifestyle: [
        "lifestyle", "health", "fitness", "travel", "food", "fashion",
        "life", "motivation", "personal", "relationship", "wellness"
      ],
      education: [
        "education", "study", "student", "learning", "tutorial",
        "course", "exam", "school", "college", "university",
        "knowledge", "guide", "how to learn"
      ],
      others: []
    };
    ...
    Object.entries(categoryKeywords).forEach(([category, keywords]) => {
      const score = keywords.reduce(
        (total, keyword) =>
          total + (value.includes(keyword.toLowerCase()) ? 1 : 0),
        0
      );
  ...
  useEffect(() => {
    ...
    // Never overwrite a category already selected while creating the blog.
    if (blogData.category) return;

    if (!categoryAutoSelectedRef.current) {
      const detectedCategory = getAutoCategory(title);
  ...
  useEffect(() => {
    ...
    const generateSubtitleAutomatically = async () => {
      const title = blogData.title.trim();
      ...
      if (
        ...
        subtitleAutoGeneratedRef.current
      ) {
        return;
      }

      subtitleAutoGeneratedRef.current = true;
      ...
      try {
        const { data } = await axios.post(
          ...
          {
            prompt: `Generate ONLY one short, professional, SEO-friendly blog subtitle for this article.
...
- Maximum 20 words.
- Make it clear, natural and engaging.`,
            title,
            subtitle: "",
            category,
          },
        ...
        if (!cancelled && data.success && data.content) {
          const generatedSubtitle = data.content
            .replace(/<[^>]*>/g, " ")
            .replace(/\s+/g, " ")
            .trim()
            .replace(/^["'`]+|["'`]+$/g, "");

          if (generatedSubtitle) {
            setBlogData((prev) => ({
              ...prev,
        ...
        if (!cancelled) {
          ...
          subtitleAutoGeneratedRef.current = false;
        }
      } finally {
        if (!cancelled) {
    ...
    generateSubtitleAutomatically();

    return () => {
      cancelled = true;
    };
  }, [blogData.title, blogData.category, blogData.subtitle]);
    ...
    const fetchBlog = async () => {
      ...
      if (blogFromRedux) {
        setBlogData({
          ...
          category: normalizeCategory(blogFromRedux.category || ""),
        });
  ...
  const blogUpdateHandler = async () => {
    ...
    if (loading || subtitleLoading) return;
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            <input
              ...
              placeholder={
                subtitleLoading
                  ? "AI is generating your subtitle..."
                  : "Enter blog subtitle"
              }
              disabled={subtitleLoading}
              className={`w-full px-4 py-2 pr-12 border rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white ${errors.subtitle ? "border-red-500" : "border-gray-300"
                }`}
            />
          ...
          {subtitleLoading && (
            <p className="text-xs text-purple-600 dark:text-purple-400 mt-1 flex items-center gap-1">
              ✨ AI is creating a professional subtitle...
            </p>
          )}
...
`;
        ...
        >
          {loading ? "Saving..." : subtitleLoading ? "Generating Subtitle..." : "Save Changes"}
        </button>
